...
import { getStoresByStatus, updateStoreActivation } from "@/lib/api/storeApi"
...
export default function AdminStores() {
    ...
    const fetchStores = async () => {
        try {
            const data = await getStoresByStatus('approved')
            setStores(data)
        } catch (error) {
            console.error('Error fetching stores:', error)
            toast.error('Failed to fetch approved stores')
        } finally {
            setLoading(false)
        }
    }
    ...
    const toggleIsActive = async (storeId, currentStatus) => {
        try {
            ...
            await updateStoreActivation(storeId, newStatus)
            // Update local state
                                ...
                                <label className="relative inline-flex items-center cursor-pointer text-gray-900">
                                    <input 
                                        ...
                                        onChange={() => toast.promise(
                                            toggleIsActive(store.id, store.isActive), 
                                            { 
