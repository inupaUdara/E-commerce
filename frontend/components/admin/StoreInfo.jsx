        ...
        <div className="flex-1 space-y-2 text-sm">
            ...
            <div className="flex items-center gap-2 text-sm ">
                <Image 
                    width={36} 
                    height={36} 
                    src={store.user?.image || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(store.user?.name || 'User')} 
                    alt={store.user?.name || 'User'} 
                    className="w-9 h-9 rounded-full" 
                />
                <div>
                    <p className="text-slate-600 font-medium">{store.user?.name || 'N/A'}</p>
