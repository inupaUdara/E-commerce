        ...
        <div className="inline-flex h-full flex-col gap-5 border-r border-slate-200 sm:min-w-60">
            <div className="flex flex-col gap-3 justify-center items-center pt-8 max-sm:hidden">
                <Image 
                    ...
                    src={storeInfo?.logo || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(storeInfo?.name || 'Store')} 
                    alt={storeInfo?.name || 'Store'} 
                    width={80} 
                    height={80} 
                />
                <p className="text-slate-700 font-medium text-center px-2">{storeInfo?.name || 'My Store'}</p>
            </div>
